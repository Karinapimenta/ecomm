...
class PaymentController{
	static async getOnePayment (req, res){
		...
		try {
			...
			return res.status(200).location(`http://localhost:3000/payments/${onePayment.id}`).json(onePayment)
		} catch (error) {
  ...
  static async savePayment(req, res){
		...
		if(flag.length == 0){
		  ...
		  const newFields = { links: [
			{
				"rel": "CANCELADO",
				method: "PATCH",
			  "href": `http://localhost:3000/payments/${newPaymentCreated.id}/CANCELADO`
			},
			{
				"rel": "CONFIRMADO",
				method: "PATCH",
				"href": `http://localhost:3000/payments/${newPaymentCreated.id}/CONFIRMADO`
			},
		]}
				await database.Payments.update(newFields, {where:{id:Number(newPaymentCreated.id)}})
		  ...
		  return res.status(201).location(`http://localhost:3000/payments/${newPaymentCreated.id}`).json(finalPayment)
		} catch (error) {
		  return res.status(500).json(error.message)
		}
		} else {
  ...
  static async updateStatus (req, res){
	...
	if(((rgxStatus).test(status)) === true ){
	  ...
	  try {
		...
		await database.Payments.update({status:status, invoiceId:newInvoice.id},{
		  where: {
		  id:Number(id)
		  }
		})
		 ...
		 return res.status(500).json(error.message)
			...
			try {
				await database.Payments.update({status:status},{
					where: {
					id:Number(id)
					}
			 })
